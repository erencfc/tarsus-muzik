...
export const sendVerificationEmail = async (email: string, token: string) => {
    const confirmLink = `https://hesapli-muzik-aletleri.vercel.app/auth/new-verification?token=${token}`;
...
export const sendPasswordResetEmail = async (email: string, token: string) => {
    const confirmLink = `https://hesapli-muzik-aletleri.vercel.app/auth/new-password?token=${token}`;
